...
class CLIPTrainer(Trainer):
    def __init__(
        ...
        if (
            not hasattr(training_args, "max_length") or training_args.max_length is None
        ...
        if data_collator is None:
            ...
            self.data_collator = CLIPDataCollator(
                processor=self.processor, max_length=max_length
            )
        else:
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            loss, similarity_logits = self.compute_loss(
                model=model, inputs=inputs, return_logits=True
            )
            loss = loss.detach()
...
if __name__ == "__main__":
    ...
    foodieclip = FoodieClip(foodieclip_config, projection_dim=64).to(device)
    # loading dataset
