...
from transformers import Trainer, TrainingArguments, PreTrainedModel, DataCollator, PreTrainedTokenizerBase, AutoProcessor
...
class IndividualClassDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return {"image":Image.open(self.path+self.img_path[idx]), "text":self.description, "text_id":self.index}
...
class CLIPDataset(Dataset):
    def __init__(self, path="", train=True, class_max_datapoints=200):
        ...
        if train:
            subpath = self.path+"data/train600x600/"
        else:
            subpath = self.path+"data/val600x600/"
        ...
        for index in self.df.index:
            ...
            path = subpath
            class_dataset = IndividualClassDataset(index=index, path=path, description=description, class_max_datapoints=class_max_datapoints)
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for index, feature in enumerate(features):
            ...
            if feature["text_id"] in texts_row_id:
                ...
                texts_row_id[feature["text_id"]] = {"row_id":len(texts)-1, "image_index":[index]}
...
if __name__ == '__main__':    
    ...
    clipdataset = CLIPDataset(path="/Users/jclkh/Desktop/Foodie_CLIP/", class_max_datapoints=class_max_datapoints)
    dataloader = DataLoader(clipdataset, batch_size=clipdataset.n_class*class_max_datapoints, collate_fn=CLIPDataCollator(processor=AutoProcessor.from_pretrained("openai/clip-vit-base-patch32")))
