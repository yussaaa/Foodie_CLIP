...
import requests
from transformers import CLIPModel, CLIPTextModel, CLIPPreTrainedModel, PreTrainedModel, CLIPVisionModel, CLIPConfig, AutoProcessor
...
class FoodieClipSimilarity(PreTrainedModel, nn.Module):
    ...
    @torch.no_grad()
    def forward(self, text_input, image_input, top_k=1, retrieve="text") -> Optional[List[List]]:
        image_embedding = image_input["embedding"]
        text_embedding = text_input["embedding"]
        image_logits = self.get_similarity(image_embedding=image_embedding, text_embedding=text_embedding)
        # get the top k best match texts for each input image
        if retrieve == "text":
            top_k_values, top_k_indices = torch.topk(image_logits, top_k, dim=-1).indices.tolist(), 
        else:
            top_k_values, top_k_indices = torch.topk(image_logits.t(), top_k, dim=-1).indices.tolist()
...
class FoodieClipImageModel(FoodieClipPreTrain):
    ...
    def __init__(self, config):
        ...
        # self.text_hidden_size = backbone.config.hidden_size
        self.visual_projection = nn.Linear(self.vision_config.hidden_size, self.config.projection_dim, bias=False)
        self.logit_scale = nn.Parameter(torch.tensor(self.config.logit_scale_init_value))
        # Initialize weights and apply final processing
        self.post_init()
...
class FoodieClip(FoodieClipPreTrain):
    ...
    def __init__(self, config, projection_dim=None):
        ...
        if projection_dim:
            ...
            self.text_projection = nn.Linear(self.config.text_config.hidden_size, self.config.projection_dim, bias=False)
            self.visual_projection = nn.Linear(self.config.vision_config.hidden_size, self.config.projection_dim, bias=False)
        else:
...
if __name__ == '__main__':
    ...
    text_input = processor(text=["a photo of a cat", "a photo of a dog"], return_tensors="pt", padding=True)
    """
    test for FoodieClip
    """
    ...
    foodieclip.save_pretrained("/Users/jclkh/Desktop/foodieclip/model")
    # loading a finetune model from /MODEL_PATH
    foodieclip = FoodieClip.from_pretrained("/Users/jclkh/Desktop/foodieclip/model")
    ...
    print(f"foodie model output {foodieclip(text_input=text_input, image_input=image_input)}")
    """
    test for FoodieClipImageModel
    """
    # loading FoodieClipImageModel from path
    foodie_image_model = FoodieClipImageModel.from_pretrained("/Users/jclkh/Desktop/foodieclip/model")
    foodie_image_model_output = foodie_image_model(image_input)
    """
    test for FoodieClipTextModel
    """
    foodie_text_model = FoodieClipTextModel.from_pretrained("/Users/jclkh/Desktop/foodieclip/model")
    ...
    print(f"text embedding consistency : {foodieclip_text_embedding == foodie_text_model_output}")
    # check the image embedding consistency between foodieclip and foodieclipimage
    print(f"image embedding consistency : {foodieclip_image_embedding == foodie_image_model_output}")
